...
function DS_sortMaterials() {
    imgData = {};
    imgList = $('.ds-productlist > li.filled');
        ...
        DS_ajax("php/ajax/material-remove.php",ajaxData,function(data){
            if(data.status.type=="success") {
                ...
                if(!checkElement($('.ds-productlist .ds-entry[data-id='+MID+']'))) {
                    $('.ds-productlist').append('<li class="ds-entry ds-square col-lg-4 col-md-4 col-sm-4 col-xs-4"><div class="ds-product-box squarebox"><div class="squarebox-inner"></div></div></li>');
                    materialCountDecrease();
                    var materialName = $(entryObj).attr('data-name');
                    var image = $('#ds-file-form .ds-file-list li img[data-name='+materialName+']');
...
function calculatePPI(actual_image_height,intial_height,height){
   var heightR = height * (393/400);
    var print_height_cm = 10;
    var result = 0;
    var ratio = actual_image_height/393;
    if(actual_image_height > 393){
...
function DS_cropImageStart() {
    ...
    function DS_cropImageDragFn(event,ui) {
        ...
        if(img_scale <= 1) {
			console.log('Hone');
			console.log('isVerical', isVertical);
			if (!isVertical){
				img_zoom_w = cbox_w * (zoomValue/100);
				img_zoom_h = (img_scale==0) ? 0 : Math.ceil(img_zoom_w/ img_scale);
			}else{
				img_zoom_h = cbox_w * (zoomValue/100);
				img_zoom_w = img_zoom_h * img_scale;
			}
        }
        // Breite Bilder
        else {				
...
function DS_cropImagePos(cropImage,isVertical) {
    ...
    console.log('cbox_w', cbox_w);
    console.log('cbox_h', cbox_h);
...
//DS_managerFileAdd();
    ...
    DS_ajax("php/ajax/manager-file-add.php",ajaxData,function(data){
        ...
        var gallery_addCode = '<div class="ds-entry-remove"><img src="/editor/img/icons/Papierkorb.png"> </div> <div class="edit-image"><img src="/editor/img/icons/Stift.png"></div><div class="squarebox-bg"></div>';
        var gallery_addCodeError = '<div class="ds-entry-remove"><img src="/editor/img/icons/Papierkorb.png"> </div> <div class="edit-image"><img src="/editor/img/icons/Stift.png"></div> <div class="warning-image" data-toggle="tooltip" title="deine aktuelle Bildauswahl ist zu groß! dadurch leidet die Druckqualität." data-placement="bottom"><img src="/editor/img/icons/Warndreieck.png"></div><div class="squarebox-bg"></div>';
        $(data.data.images).each(function(index,material){
            if(material.status == "success") {
                ...
                imageRefDate = new Date();
                material.thumb += "?"+imageRefDate.getTime();
                gallery_box = $("<div/>");
